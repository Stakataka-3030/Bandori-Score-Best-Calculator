...
function getComboMultiplier(combo: number): number {
  ...
  if (combo <= 150) return 1.03;
  if (combo <= 200) return 1.04;
  if (combo <= 250) return 1.05;
  if (combo <= 300) return 1.06;
  if (combo <= 400) return 1.07;
  if (combo <= 500) return 1.08;
  if (combo <= 600) return 1.09;
  if (combo <= 700) return 1.1;
  return 1.11;
}
function getMedleyComboMultiplier(combo: number): number {
  ...
  if (combo <= 3000) return 1.04 + Math.floor((combo - 1) / 100) * 0.01;
  return 1.34;
}
...
function addChartNote(notes: Array<{ beat: number; skill: boolean }>, note: unknown): void {
  ...
  notes.push({ beat, skill: "skill" in note });
}
function parseChartNotes(chart: BestdoriChartEntity[]): {
  ...
  chart.forEach((entity) => {
    switch (entity.type) {
      case "Single":
      case "Directional": addChartNote(notes, entity); break;
      case "Long": {
      ...
      case "Slide": {
        ...
        connections.forEach((connection, index) => {
          if (index > 0 && index < connections.length - 1 && isRecord(connection) && "hidden" in connection) return;
      ...
      case "System":
        if (entity.data === "cmd_fever_start.wav") feverStartBeat = toFiniteNumber(entity.beat, 0);
        else if (entity.data === "cmd_fever_end.wav") feverEndBeat = toFiniteNumber(entity.beat, 0);
        break;
      default: break;
    }
  });
  notes.sort((left, right) => left.beat - right.beat || Number(right.skill) - Number(left.skill));
  bpms.sort((left, right) => left.beat - right.beat);
  return { notes, bpms, feverStartBeat, feverEndBeat };
}
export function prepareBandoriChart(chart: BestdoriChartEntity[], song: BestdoriSongMaster, difficulty: BandoriTeamSearchDifficulty): PreparedChart {
  const parsed = parseChartNotes(chart);
  let bpmIndex = 0; let currentBpm = 1; let currentTime = 0; let previousBeat = 0;
  ...
  parsed.notes.forEach((note) => {
    while (bpmIndex < parsed.bpms.length && parsed.bpms[bpmIndex].beat < note.beat) {
      ...
      previousBeat = bpm.beat; currentBpm = bpm.bpm; bpmIndex += 1;
    }
    ...
    preparedNotes.push({
      ...
      time: currentTime,
      skill: note.skill,
      fever: parsed.feverStartBeat !== null && parsed.feverEndBeat !== null && note.beat >= parsed.feverStartBeat && note.beat <= parsed.feverEndBeat,
    });
  });
  const skillStartNotes: number[] = []; const skillTriggerTimes: number[] = [];
  preparedNotes.forEach((note, index) => { if (note.skill) { skillStartNotes.push(index + 1); skillTriggerTimes.push(note.time); } });
  return { notes: preparedNotes, playLevel: getSongPlayLevel(song, difficulty), notesCount: preparedNotes.length, skillStartNotes, skillTriggerTimes };
}
function setPreparedChartFeverEnabled(chart: PreparedChart, enabled: boolean): PreparedChart {
  return enabled ? chart : { ...chart, notes: chart.notes.map((note) => ({ ...note, fever: false })) };
}
export function getCachedPreparedChart(input: BandoriTeamSearchInput): PreparedChart {
  ...
  const cacheKey = input.chartCacheKey ? `${input.chartCacheKey}:fever=${useFever ? "1" : "0"}` : null;
  if (cacheKey) { const cached = preparedChartCache.get(cacheKey); if (cached) return cached; }
  const chart = setPreparedChartFeverEnabled(prepareBandoriChart(input.chart, input.song, input.difficulty), useFever);
  if (cacheKey) {
    ...
    if (preparedChartCache.size > PREPARED_CHART_CACHE_LIMIT) {
      ...
      if (oldestKey) preparedChartCache.delete(oldestKey);
    }
  }
  return chart;
}
