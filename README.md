...
我们在实际对比中发现，部分现有计算逻辑与游戏实际行为存在差异，主要包括以下两个方面：

### 1. 技能触发顺序概率问题

游戏内前五个技能的触发顺序并不是简单的 `5! = 120` 种排列均匀随机。

经过 HHWX 作者 **BluewaterAlnilamII** 的分析发现，游戏实际使用的技能顺序随机算法并非标准 Fisher-Yates 洗牌算法，而是通过逐个取出成员并随机插回剩余列表中的方式实现。这会导致：

- 部分理论排列永远不会出现；
- 可达排列数量少于 120 种；
- 不同技能顺序具有不同概率。

因此，简单地将 120 种排列全部视为 `1/120` 会产生误差。本项目根据实际随机过程计算可达技能顺序及其概率，并使用加权期望计算最终结果。

感谢 **BluewaterAlnilamII（HHWX 作者）** 对该问题的发现与分析，该分析也是本项目修正技能概率模型的重要依据。

### 2. 技能触发时间窗口问题

部分计算器会直接使用谱面中的标称技能触发时间，但游戏内技能实际上不能重叠：如果前一个技能仍在持续，后一个技能会被推迟到前一个技能结束后的 **0.75 秒**再触发。
...
- Medley 三队最优搜索。
- 使用实际的技能顺序概率模型，而不是假设所有排列均匀随机。
- 正确处理技能延迟、递归后移以及技能时间线变化。
